...
async function writeResultsToDatabase(results, config, createdAt = new Date()) {
    ...
    if (config.tests && !Array.isArray(config.tests)) {
        ...
        logger_1.default.debug(`Converting non-array test configuration to empty array for database storage. Type: ${testsType}, hasPath: ${hasPath}`);
    }
    promises.push(db
    ...
    for (const eval_ of evals_) {
        ...
        if (testCases && predicate(resultWrapper)) {
            ...
            if (typeof testCases === 'string') {
                ...
                logger_1.default.warn('Skipping TestGeneratorConfig object in database storage');
                continue;
            }
            const datasetId = (0, createHash_1.sha256)(JSON.stringify(storableTestCases));
