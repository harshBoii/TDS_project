    ...
    if (!ret.prompts) {
        ...
        !ret.tests ||
            ...
            (Array.isArray(ret.tests) &&
                ret.tests.some((test) => isTestCaseWithVars(test) && Object.keys(test.vars || {}).includes('prompt')));
        if (!hasAnyPrompt) {
...
async function combineConfigs(configPaths) {
    ...
    for (let i = 0; i < configs.length; i++) {
        ...
        if (typeof config.tests === 'string') {
            const newTests = await (0, testCaseReader_1.readTests)(config.tests, path.dirname(configPath));
            tests.push(...newTests);
        }
...
async function resolveConfigs(cmdObj, _defaultConfig, type) {
    ...
    type !== 'DatasetGeneration' &&
        !hasProviders) {
        logger_1.default.error('You must specify at least 1 provider (for example, openai:gpt-4.1)');
        process_1.default.exit(1);
    }
