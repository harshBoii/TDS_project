    ...
    async runAttack({ prompt, filters, vars, provider, context, options, test, }) {
        ...
        `;
                ...
                if (test && assertToUse) {
                    ...
                    if (grader) {
                        const { grade } = await grader.getResult(attackPrompt, lastResponse.output, test, provider, assertToUse && 'value' in assertToUse ? assertToUse.value : undefined);
                ...
                if (evalPercentage === 100) {
                    ...
                    logger_1.default.debug(`[Crescendo] Jailbreak Successful via Internal Evaluator, EXITING at ROUND ${roundNum}`);
                    break;
                }
                if (graderPassed === false) {
                    exitReason = 'Grader failed';
                    logger_1.default.debug(`[Crescendo] Jailbreak Successful via External Grader, EXITING at ROUND ${roundNum}`);
                    break;
                }
        ...
        if (roundNum >= this.maxRounds && exitReason === 'Max rounds reached') {
            logger_1.default.debug(`[Crescendo] Max rounds (${this.maxRounds}) reached. Attack completed.`);
        }
        if (evalFlag) {
