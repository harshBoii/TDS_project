...
exports.addImageToBase64 = addImageToBase64;
...
async function importSharp() {
    ...
    try {
        ...
        sharpCache = await Promise.resolve().then(() => __importStar(require('sharp')));
        return sharpCache;
    }
    catch (error) {
        logger_1.default.warn(`Sharp library not available: ${error}`);
        return null;
    }
}
/**
...
async function textToImage(text) {
    ...
    `;
        ...
        logger_1.default.error(`Error generating image from text: ${error}`);
 ...
 */
async function addImageToBase64(testCases, injectVar) {
    ...
    for (const testCase of testCases) {
        (0, invariant_1.default)(testCase.vars, `Image encoding: testCase.vars is required, but got ${JSON.stringify(testCase)}`);
        ...
        if (progressBar) {
            ...
            logger_1.default.debug(`Processed ${imageTestCases.length} of ${testCases.length}`);
        }
    }
    if (progressBar) {
        progressBar.stop();
...
async function main() {
    ...
    logger_1.default.info(`Converting text to image: "${textToConvert}"`);
    try {
        ...
        logger_1.default.info(`Base64 image (first 100 chars): ${base64Image.substring(0, 100)}...`);
        logger_1.default.info(`Total base64 image length: ${base64Image.length} characters`);
        ...
        if (require.main === module) {
            ...
            logger_1.default.info(`Image file written to: ${outputFilePath}`);
            logger_1.default.info(`You can open it with any image viewer to verify the conversion.`);
        }
    }
    catch (error) {
        logger_1.default.error(`Error generating image from text: ${error}`);
