...
async function doGenerateRedteam(options) {
    ...
    if (configPath) {
        ...
        (0, invariant_1.default)(providers.length === 1, 'Generation can only be run with a single provider. Please specify a single provider in the config file.');
        if (!(0, envars_1.getEnvBool)('PROMPTFOO_DISABLE_REDTEAM_TARGET_DISCOVERY_AGENT', true) &&
            ...
            try {
                ...
                logger_1.default.info('Starting Target Discovery Agent');
                targetPurposeDiscoveryResult = await (0, discover_1.doTargetPurposeDiscovery)(providers[0], testSuite.prompts[0]);
            }
            catch (error) {
                logger_1.default.error(`Target Discovery Agent failed from error, skipping: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        try {
            // If the provider is a cloud provider, check for plugin severity overrides:
            const providerId = (0, providers_1.getProviderIds)(resolved.config.providers)[0];
    ...
    if (options.output && options.output.endsWith('.burp')) {
        ...
        ret = (0, manage_1.writePromptfooConfig)(updatedYaml, options.output, headerComments);
        ...
        ret = (0, manage_1.writePromptfooConfig)(existingConfig, configPath, headerComments);
        ...
        ret = (0, manage_1.writePromptfooConfig)({ tests: redteamTests }, 'redteam.yaml', headerComments);
    }
    telemetry_1.default.record('command_used', {
