...
async function doTargetPurposeDiscovery(target, prompt) {
    ...
    const pbar = new cli_progress_1.default.SingleBar({
        format: `Mapping the target {bar} {percentage}% | {value}${exports.DEFAULT_TURN_COUNT ? '/{total}' : ''} turns`,
        barCompleteChar: '\u2588',
        barIncompleteChar: '\u2591',
        hideCursor: true,
        gracefulExit: true,
    ...
    while (!done && turn < exports.MAX_TURN_COUNT) {
        try {
            ...
            state = data.state;
            if (data.error) {
                logger_1.default.error(`${LOG_PREFIX} Error from remote server: ${data.error}`);
            }
            // Should another question be asked?
                ...
                if (targetResponse.error) {
                    logger_1.default.error(`${LOG_PREFIX} Error from target: ${targetResponse.error}`);
                    if (turn > exports.MAX_TURN_COUNT) {
                        logger_1.default.error(`${LOG_PREFIX} Too many retries, giving up.`);
                        return undefined;
                    }
                    continue;
                }
...
function mergeTargetPurposeDiscoveryResults(humanDefinedPurpose, discoveryResult) {
    ...
    const hasDiscoveredContent = discoveryResult &&
