...
async function evaluate(testSuite, options = {}) {
    ...
    const loadedProviders = await (0, providers_1.loadApiProviders)(testSuite.providers, {
        env: testSuite.env,
    });
    const providerMap = {};
    ...
    if (constructedTestSuite.defaultTest?.options?.provider) {
        constructedTestSuite.defaultTest.options.provider = await (0, providers_1.resolveProvider)(constructedTestSuite.defaultTest.options.provider, providerMap, { env: testSuite.env });
    }
    for (const test of constructedTestSuite.tests || []) {
        if (test.options?.provider) {
            test.options.provider = await (0, providers_1.resolveProvider)(test.options.provider, providerMap, {
                env: testSuite.env,
            });
        }
        if (test.assert) {
            for (const assertion of test.assert) {
                ...
                if (assertion.provider) {
                    assertion.provider = await (0, providers_1.resolveProvider)(assertion.provider, providerMap, {
                        env: testSuite.env,
                    });
                }
            }
