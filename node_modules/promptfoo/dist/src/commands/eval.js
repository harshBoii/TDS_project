...
async function doEval(cmdObj, defaultConfig, defaultConfigPath, evaluateOptions) {
    ...
    const runEvaluation = async (initialization) => {
        ...
        let maxConcurrency = cmdObj.maxConcurrency ?? evaluateOptions.maxConcurrency ?? evaluator_1.DEFAULT_MAX_CONCURRENCY;
            ...
            if (providerIds.length > 0) {
                ...
                for (const { id, usage } of sortedProviders) {
                    if ((usage.total || 0) > 0 || (usage.prompt || 0) + (usage.completion || 0) > 0) {
                        ...
                        logger_1.default.info(`    ${chalk_1.default.gray(displayId + ':')} ${chalk_1.default.white(displayTotal.toLocaleString())}`);
