...
async function listAssistantFiles(context, assistantId, options = { requestOptions: {} }) {
    const result = await _listAssistantFilesSend(context, assistantId, options);
    return _listAssistantFilesDeserialize(result);
}
function _createThreadSend(context, body, options = { requestOptions: {} }) {
    var _a;
    return context.path("/threads").post(Object.assign(Object.assign({}, coreClient.operationOptionsToRequestParameters(options)), { body: {
...
function _uploadFileSend(context, file, purpose, options = { requestOptions: {} }) {
    var _a;
    return context.path("/files").post(Object.assign(Object.assign({}, coreClient.operationOptionsToRequestParameters(options)), { contentType: (_a = options.contentType) !== null && _a !== void 0 ? _a : "multipart/form-data", body: {
            file: coreRestPipeline.createFile(file, (options === null || options === void 0 ? void 0 : options.filename) || "unknown.txt"),
            purpose: purpose,
        } }));
}
...
function parseRunStepOutput(runStepOutput) {
    const { step_details, last_error, created_at, expired_at, completed_at, cancelled_at, failed_at } = runStepOutput, rest = tslib.__rest(runStepOutput, ["step_details", "last_error", "created_at", "expired_at", "completed_at", "cancelled_at", "failed_at"]);
